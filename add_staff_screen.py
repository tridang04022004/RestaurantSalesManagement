...
from db_connection import add_staff
from PIL import Image
import os

class AddStaffScreen:
    def __init__(self, root, role):
        self.root = root
        self.role = role
        self.root.title("Restaurant Management - Add New Staff")
        window_width = 1600
        window_height = 900
    ...
    def show_add_staff_form(self):
        ...
        new_username_var = ctk.StringVar()
        ctk.CTkLabel(
            form_frame,
            text="Username",
        ...
        add_button = ctk.CTkButton(
            button_frame,
            text="Add Staff",
            command=lambda: self.add_staff(new_username_var.get(), new_password_var.get()),
    ...
    def add_staff(self, username, password):
        if not username or not password:
            ...
            return
        try:
            if add_staff(username, password):
                messagebox.showinfo("Success", f"Staff account '{username}' added successfully!")
