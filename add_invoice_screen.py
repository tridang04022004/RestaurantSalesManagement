...
class AddInvoiceScreen:
    def __init__(self, root, role):
        self.root = root
        self.role = role
        self.root.title("Restaurant Management - Add New Invoice")
        ...
        self.font_button = ("Helvetica", 18, "bold")  # New font for Save and Cancel buttons
    ...
    def show_add_invoice(self):
        print("Starting show_add_invoice...")
        ...
        self.search_entry = ctk.CTkEntry(search_frame, width=400, height=40, font=self.font_medium)  # Increased width
        ...
        save_button = ctk.CTkButton(
            ...
            height=50,
            font=self.font_button,  # Updated font to bigger and bold
            fg_color=self.blue_accent
        )
    ...
    def get_available_tables(self):
        """Fetch available tables (status = 'empty')."""
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("SELECT id, table_number FROM tables WHERE status = 'empty'")
    ...
    def save_invoice(self):
        ...
        if invoice_type == "dine in":
            ...
            try:
                conn = get_db_connection()
                cursor = conn.cursor()
                cursor.execute("SELECT id FROM tables WHERE table_number = ? AND status = 'empty'", (int(table_number),))
        ...
        try:
            ...
            cursor.execute(
                "INSERT INTO invoices (table_id, total_amount, timestamp, type, status) VALUES (?, ?, ?, ?, 'unpaid')",
                (table_id, self.total_amount, timestamp, invoice_type)
            )
