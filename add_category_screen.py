...
from db_connection import add_category
...
class AddCategoryScreen:
    def __init__(self, root, role):
        self.root = root
        self.role = role
        self.root.title("Restaurant Management - Add New Category")
        window_width = 1600
        window_height = 900
    ...
    def show_add_category_form(self):
        ...
        add_button = ctk.CTkButton(
            button_frame,
            text="Add Category",
            command=lambda: self.add_category(self.new_category_var.get()),
            font=self.font_button,
            width=200,
            height=50,
        ...
        add_button.bind("<Enter>", lambda event, b=add_button: self.on_button_enter(event, b, self.accent_color, self.accent_dark))
        add_button.bind("<Leave>", lambda event, b=add_button: self.on_button_leave(event, b, self.accent_color, self.accent_dark))
    ...
    def select_image(self):
        ...
        if file_path:
            ...
            self.show_message("Image selected successfully!", self.success_color)
            try:
                ...
                self.show_message(f"Failed to load image: {e}", self.error_color)
    ...
    def add_category(self, name):
        ...
        if not self.image_path:
            self.show_message("Please select an image", self.error_color)
            return
        ...
        try:
            ...
            if add_category(name, destination_path):
                self.show_message(f"Category '{name}' added successfully!", self.success_color)
                self.root.after(2000, self.back_to_menu)
            else:
                self.show_message("Category name already exists", self.error_color)
