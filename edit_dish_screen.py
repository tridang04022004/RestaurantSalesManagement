...
class EditDishScreen:
    def __init__(self, root, role, category_id, category_name, dish_id, dish_name, price, img_path):
        self.root = root
        self.role = role
        ...
        self.root.title(f"Restaurant Management - Edit Dish in {category_name}")
        window_width = 1600
        window_height = 900
        ...
        try:
            self.show_edit_dish(dish_name, price)
    ...
    def show_edit_dish(self, dish_name, price):
        ...
        ctk.CTkLabel(
            top_bar,
            text=f"Edit Dish in {self.category_name}",
        ...
        self.dish_name_entry.insert(0, dish_name)
    ...
    def load_current_image(self):
        try:
            ...
            messagebox.showerror("Error", f"Failed to load current image: {e}")
    ...
    def save_dish(self):
        ...
        if self.new_image_path:
            try:
                ...
                img_extension = os.path.splitext(self.new_image_path)[1]
                img_filename = f"img/{timestamp}{img_extension}"
                os.makedirs("img", exist_ok=True)
                image = Image.open(self.new_image_path)
                image.save(img_filename)
                img_path_to_save = img_filename
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save new image: {e}")
                return
        try:
            from db_connection import update_dish
            success, old_img_path = update_dish(self.dish_id, dish_name, price, category_id, img_path_to_save)
            if success:
                if self.new_image_path and old_img_path and os.path.exists(old_img_path):
                    os.remove(old_img_path)
                messagebox.showinfo("Success", "Dish updated successfully!")
                self.back_to_dishes()
            else:
                messagebox.showerror("Error", "Failed to update dish.")
                if self.new_image_path and os.path.exists(img_path_to_save):
                    os.remove(img_path_to_save)
        except Exception as e:
            messagebox.showerror("Error", f"Failed to update dish: {e}")
            if self.new_image_path and os.path.exists(img_path_to_save):
                os.remove(img_path_to_save)
